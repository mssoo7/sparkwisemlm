    ...
    <table class="table table-bordered">
      <thead class="thead-dark ">
        <tr class="font-weight-bold">
          ...
          <th wire:click="sortBy('renew_status')">Status
            @include('partials.sort-icon',['field'=>'renew_status'])
          </th>
        </tr>
      </thead>
      <tbody>
      ...
      @foreach ($myteam as $value)
        ...
        <tr @if ($value->renew_status==1) class="bg-success" @else class="bg-warning" @endif>
          
          <td>{{$i++}}</td>
          <td>{{$value->name}}</td>
          <td>@if ($value->renew_status==1) Active @else Hold @endif</td>
